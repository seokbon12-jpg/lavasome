...
셰이더는 21st.dev Shader Builder 레시피를 바탕으로 하되, **"심플하게" 요청에 맞춰
잔물결과 뒤엉킴을 덜어낸 값**입니다(원본 값은 `shader-bg.js` 상단 주석에 병기).
커서 반응은 꺼져 있습니다.
...
**대비는 렌더 픽셀로 측정했습니다.** 글자가 없는 빈 띠에서 배경을 뽑아 계산했고,
흰 글자 기준 **9.7~12.9:1** 로 WCAG AAA(7:1)를 넘깁니다.
물을 단순화하면서 밝은 소용돌이가 사라져 이 값이 더 올라갔습니다.
물을 다시 밝히거나 무늬를 되살리면 이 값이 먼저 떨어집니다.
...
**무늬의 복잡도**는 `shader-bg.js` 안 네 곳입니다. 괄호는 원본(복잡한) 값.

| 위치 | 현재 | 원본 | 올리면 |
|---|---|---|---|
| `fbm()` 의 옥타브 수 | 3 | 5 | 잔물결이 생깁니다 |
| `shade()` 의 두 번째 접힘 계수 | 1.6 | 3.0 | 대리석처럼 뒤엉킵니다 |
| `SHAPE[1]` intensity | 0.34 | 0.60 | 흐름이 거세집니다 |
| `SHAPE[0]` scale | 1.30 | 1.72 | 무늬가 잘게 쪼개집니다 |

속도는 `TIME_SCALE`(현재 `0.72`, 원본 `0.97`), 색은 `COLORS` 입니다.
물을 밝히거나 무늬를 되살리면 흰 글자 대비가 먼저 떨어지니 함께 확인하세요.
