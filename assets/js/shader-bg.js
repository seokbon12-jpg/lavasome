...
(function () {
  ...
  var FRAG = [
    ...
    '}',
    '',
    /* 옥타브 4 — 원 레시피는 5였다. 5는 잔물결이 자글거렸고, 3은
       큰 덩어리만 남아 물이라기보다 연기에 가까웠다. 4가 결이
       보이면서도 조용한 지점이다. */
    'float fbm(vec2 p) {',
    '  float v = 0.0;',
    '  float a = 0.5;',
  ...
  /* 레시피 — 21st.dev 기반, "심플하게" 요청에 맞춰 조정.
     ...
     원본 대비 바뀐 값 ▸ 되돌리려면 괄호 안 값으로.
       fbm 옥타브    4    (5)     잔물결만 덜어냄
       접힘 계수     1.6  (3.0)   결 펴기
       intensity     0.40 (0.60)  뒤엉킴 완화
       scale         1.30 (1.72)  무늬를 크게 — 덩어리 수가 줄어든다
       contrast      1.16 (1.22)  빛줄기 경계를 조금 세움
  ...
  var TIME_SCALE = 0.72;
  var SHAPE     = [1.30, 0.40, 0.50, 0.00];
  var SURFACE   = [2.40, 1.16, 0.00, 1.00];
